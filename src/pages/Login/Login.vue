...
export default {
  data () {
    return {
      loginWay: true, // true代表短信登录 false就代表密码登录
      computeTime: 0, // 计时的时间
      phone: '', // 手机号
      code: '', // 短信验证码
      name: '', // 用户名
  ...
  // 异步获取短信验证码
    async getCode () {
    // 如果当前没有计时 this.computeTime === 0 也可以写作 !this.computeTime
    ...
    // 获取新的图片验证码
    getCaptcha () {
    // 每次制定的src要不一样 用?time + Date.now 时间不一样 让路径变化 /api/captcha = http://localhost:4000
